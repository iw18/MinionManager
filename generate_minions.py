...
r = requests.get("https://hackicims.com/api/v1/companies/144/people", headers=headers)
#print(r.text)

r = json.loads((requests.get("https://hackicims.com/api/v1/companies/144/jobs", headers=headers)).text)
i = 0
for job in r:
    job["title"] = job_titles[i]
    i = i + 1
    payload = json.JSONEncoder().encode(job)
    r = requests.put("https://hackicims.com/api/v1/companies/144/jobs/" + str(job["id"]), data=payload, headers=headers)

r = json.loads((requests.get("https://hackicims.com/api/v1/companies/144/people", headers=headers)).text)
for person in r:
    ...
    r = requests.put("https://hackicims.com/api/v1/companies/144/people/" + str(person["id"]), data=payload, headers=headers)

p = json.loads((requests.get("https://hackicims.com/api/v1/companies/144/people", headers=headers)).text)
j = json.loads((requests.get("https://hackicims.com/api/v1/companies/144/jobs", headers=headers)).text)
